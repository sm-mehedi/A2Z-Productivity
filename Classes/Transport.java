...
public class Transport extends JFrame implements ActionListener {
    ...
    bg1 = new ImageIcon("Elements/ticket.jpg");
        ...
        if(ae.getSource()==b1)
        {
            if(ae.getSource() == b1)
            {
                ...
                else
                {
                    try 
                    {
                        File file = new File("D:\\A2Z Productivity\\Data\\bd_Ticket_signin.txt");
                        if (!file.exists()) 
                        {
                            file.createNewFile();
                        }
